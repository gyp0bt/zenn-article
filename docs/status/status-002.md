...
- 伊能忠敬計画の確定事項（ユーザー確認済み）:
  - Tinder Platinum（フル課金）: Passport無制限変更、即時反映
  - 探索半径: 固定値ではなく決定変数（最大約160km）
  - スワイプレート: 最大1回/秒（BAN回避のハードリミット）
  - 先行実験ログ: 残存なし → 再生成レートは公開統計＋再実験から推定
  - Tinder API不使用。ブラウザ自動化（Puppeteer）のみ
  - 制限事項は2026年現行仕様を採用
